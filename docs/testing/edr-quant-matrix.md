...
2026-08-28 closeout（`windows-closeout` / nyx-ci-test run [33135025777](https://github.com/qiaozhiyi/nyx-ci-test/actions/runs/33135025777)）：同一 job 扩到 8 行。vad / threadless 安全前缀 / fluctuation scratch 均为 100%、0 告警。pool_party 仍 0.0%——08-28 先是 notepad 无 TP（缺 sleeper，Windows CI `inject_pool` 出口 0x9）；sleeper 部署后仍 0x9；in-process TP 后出口 1，marker `refuse self-inject`。现改 `nyx-bof-isolated-probe.exe --hold-tp` 子进程当靶。不是产品失败；下次 hosted 跑才是实测，本表不预填 100%。`env_limit` 仍是 Defender 已关闭。完整 operator RIP 劫持 / 真 beacon `sleep` 仍需桌面 C2 会话（§5）。
...
| 2026-08-24 | pool_party | Microsoft Defender 4.18.26070.9 | exe | 1 | 0.0% | 0 | Defender 已关闭 | 同上；08-24 exit 0x5 WARN 降级（句柄表偏移已修）；08-28 0.0% 先是 notepad 无 TP，后是 `refuse self-inject`；`--hold-tp` 子进程后的下次 hosted 跑才是实测 |
