...
**日期:** 2026-08-04
**状态:** 已批准(用户逐节确认);WP-A/WP-B1/WP-B2/WP-C/B3 全部实施(2026-08-08)
...
- 全局唯一项(`#[global_allocator]`、`#[panic_handler]`、`#[alloc_error_handler]`)只能留壳;build.rs 烘焙逻辑随迁或改壳生成。**【2026-08-08 实施修订】** `server_pub` include 不留壳:实测 `config.rs` (core) 消费 `SERVER_PUB`,留壳构成 core→壳反向边;已将 bake_server_pub + include 整体下沉 `nyx-implant-core`(config.rs 自己 include),烘焙逻辑按消费方三分——config_blob+server_pub → core、envelopes → net(生成代码 `crate::heap` 路径改 `nyx_implant_core::heap`)、bake_offsets 留壳(该烘焙自始无消费方,HEAD 既有,未动)。**【2026-08-08 实施修订 2】** `bake_offsets`(implant-win build.rs 的 kernel-offsets 烘焙,`NYX_OFFSETS` 驱动)随后在 AH-13 clippy 债清理中移除:HEAD 与工作树均无 `include!` 消费方,偏移单一来源为 `nyx_implant_evasionsdk::offsets_table` 运行时表(与整改计划"偏移单一来源"一致);相关历史文档引用已同步。另:`task_guard.rs`(WP-B1 新增)归属 tasks crate,补记于此。
