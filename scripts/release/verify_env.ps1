# verify_env.ps1 — release-pipeline preflight: confirm the release runner is in
# the locked-down Defender posture the release spec requires before any artifact
# is built. On GitHub-hosted runners the Defender cmdlets may be unavailable;
# the MAPS/Exclusion checks then degrade to warnings (the build runs in
# GitHub's sandbox, not the engagement VPS) instead of aborting.
#
...
#
# Assumes CWD = repo root. PowerShell 5.1 / pwsh 7 both work.
...
try {
    ...
    Write-Host '::warning::Get-MpPreference unavailable on this host (e.g. GitHub-hosted runner) — Defender posture not verifiable; skipping MAPS + Exclusion checks (hosted sandbox).'
    Write-Host '== verify_env OK (hosted runner: Defender posture not applicable) =='
    exit 0
}
$maps = $mp.MAPSReporting
...
$current = @($mp | Select-Object -ExpandProperty ExclusionPath)
