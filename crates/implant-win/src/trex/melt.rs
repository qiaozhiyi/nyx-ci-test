...
type NtFreeFn = unsafe extern "system" fn(
...
pub unsafe fn wipe_and_free_pages(pages: &[*mut c_void]) {
    let protect = match resolve_nt_protect() { Some(f) => f, None => return };
    let free = match resolve_nt_free() { Some(f) => f, None => return };
    let cur: isize = -1;
...
unsafe fn resolve_nt_close() -> Option<NtCloseFn> {
    crate::resolve::export_addr(b"ntdll.dll", b"NtClose")
        .map(|a| core::mem::transmute(a))
}

/// Close all tracked NT handles. Skips pseudo-handles (-1, -2).
pub unsafe fn close_all_handles(handles: &[isize]) {
    let nt_close = match resolve_nt_close() { Some(f) => f, None => return };
    for &h in handles {
        if h == -1 || h == -2 { continue; } // skip pseudo-handles
        nt_close(h);
    }
}
...
unsafe fn resolve_nt_terminate() -> Option<NtTerminateFn> {
    crate::resolve::export_addr(b"ntdll.dll", b"NtTerminateThread")
        .map(|a| core::mem::transmute(a))
}
