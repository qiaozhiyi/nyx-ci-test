...
pub struct SessionGraph {
    ...
    list: FlatList,
    #[rust(1.0)] zoom: f64,
    #[rust(dvec2(0.0, 0.0))] pan: DVec2,
    #[rust] drag_start: Option<DVec2>,
    #[rust] pan_start: DVec2,
}
...
impl Widget for SessionGraph {
        ...
        for (i, session) in sessions.iter().enumerate() {
            ...
            if let Some(mut hline) = self.list.item(cx, LiveId::from_num(1, i as u64), id!(HLine)) {
                ...
                let _ = hline.draw_walk(cx, scope, Walk {
                    abs_pos: Some(dvec2((ts_x + n_width + 20.0) as f64, line_y as f64) * self.zoom + self.pan),
            ...
            if let Some(mut node) = self.list.item(cx, LiveId::from_num(2, i as u64), id!(Node)) {
                ...
                let tag = if is_win { "[WIN]" } else if is_mac { "[MAC]" } else { "[NIX]" };
                ...
                lbl.set_text(cx, &format!("{}{}@{}", session.username, admin_star, session.hostname));
                script_apply_eval!(cx, lbl, { draw_text +: { color: #(p.primary) } });
            ...
            if last_sy >= first_sy {
                ...
                if let Some(mut ts_stub) = self.list.item(cx, id!(ts_stub), id!(HLine)) {
                    ...
                    let _ = ts_stub.draw_walk(cx, scope, Walk {
                        abs_pos: Some(dvec2((ts_x + n_width) as f64, ts_mid_y as f64) * self.zoom + self.pan),
                ...
                if let Some(mut ts_vline) = self.list.item(cx, id!(ts_vline), id!(VLine)) {
                    ...
                    let _ = ts_vline.draw_walk(cx, scope, Walk {
                        abs_pos: Some(dvec2((ts_x + n_width + 20.0) as f64, min_y as f64) * self.zoom + self.pan),
                        width: Size::Fixed((2.0 * self.zoom).max(1.0)),
                        height: Size::Fixed((max_y - min_y) as f64 * self.zoom + (2.0 * self.zoom).max(1.0)),
                        ..Walk::default()
                    });
                }
            }
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        match event.hits(cx, area) {
            ...
            Hit::FingerScroll(fs) => {
                let scroll = if fs.scroll.y != 0.0 { fs.scroll.y } else { fs.scroll.x };
                if scroll != 0.0 {
