...
unsafe fn resolve_nt_free() -> Option<NtFreeFn> {
    nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtFreeVirtualMemory")
        .map(|a| core::mem::transmute(a))
}
...
unsafe fn resolve_nt_close() -> Option<NtCloseFn> {
    nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtClose")
        .map(|a| core::mem::transmute(a))
}
...
unsafe fn resolve_nt_terminate() -> Option<NtTerminateFn> {
    nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtTerminateThread")
        .map(|a| core::mem::transmute(a))
}
