...
pub extern "C" fn nyx_bof_reset() {
    ...
    unsafe { core::ptr::write((&raw mut OUT[0]) as *mut u8, 0); }
}
...
pub extern "C" fn nyx_bof_output() -> *const c_char {
    ...
    if len < OUT_CAP {
        unsafe { core::ptr::write((&raw mut OUT[len]) as *mut u8, 0); }
    }
...
fn format_into(args: &[u64; 4], fmt: *const c_char) {
    ...
    loop {
        ...
        if b != b'%' { push_byte(b); continue; }
        let spec = unsafe { *fmt.add(fi) as u8 };
        if spec == 0 { push_byte(b'%'); break; }
        fi += 1;
        match spec {
            b'%' => push_byte(b'%'),
            b's' => {
            ...
            b'd' | b'i' => { if ai < 4 { push_i32(args[ai] as i32); ai += 1; } }
            b'x' => { if ai < 4 { push_hex(args[ai] as u32); ai += 1; } }
            b'c' => { if ai < 4 { push_byte((args[ai] & 0xFF) as u8); ai += 1; } }
            _ => { push_byte(b'%'); push_byte(spec); }
        }
    }
}
...
fn push_byte(b: u8) {
    ...
    if len < OUT_CAP {
        unsafe { core::ptr::write((&raw mut OUT[len]) as *mut u8, b); }
        OUT_LEN.store(len + 1, Ordering::Release);
    }
}

fn push_i32(v: i32) {
    if v == 0 { push_byte(b'0'); return; }
    let mut buf = [0u8; 12];
    let mut neg = false; let mut n = v;
    if v < 0 { neg = true; n = -v; }
    let mut pos = buf.len();
    while n > 0 && pos > 0 { pos -= 1; buf[pos] = b'0' + ((n % 10) as u8); n /= 10; }
    if neg && pos > 0 { pos -= 1; buf[pos] = b'-'; }
    for i in pos..buf.len() { push_byte(buf[i]); }
}

fn push_hex(v: u32) {
    if v == 0 { push_byte(b'0'); return; }
    let mut buf = [0u8; 8];
    let mut n = v; let mut pos = buf.len();
    while n > 0 && pos > 0 {
        pos -= 1;
        let d = (n & 0xF) as u8;
