...
impl Context {
    /// Const-construct a zeroed CONTEXT. `Default` is not const, but statics
    /// (task_guard's snapshot slot) must initialize a `SyncCell<Context>` at
    /// compile time.
    ...
    fn read_u64(&self, off: usize) -> u64 {
        // Panic-free under WP-B2: offsets are WinNT.h constants well inside
        // the 1232-byte buffer, but an OOB read must yield 0, never abort.
        let mut b = [0u8; 8];
        if let Some(s) = self.buf.get(off..off + 8) {
            b.copy_from_slice(s);
        }
        u64::from_le_bytes(b)
    }
    fn write_u64(&mut self, off: usize, v: u64) {
        if let Some(s) = self.buf.get_mut(off..off + 8) {
            s.copy_from_slice(&v.to_le_bytes());
        }
    }
    fn read_u32(&self, off: usize) -> u32 {
        let mut b = [0u8; 4];
        if let Some(s) = self.buf.get(off..off + 4) {
            b.copy_from_slice(s);
        }
        u32::from_le_bytes(b)
    }
    fn write_u32(&mut self, off: usize, v: u32) {
        if let Some(s) = self.buf.get_mut(off..off + 4) {
            s.copy_from_slice(&v.to_le_bytes());
        }
    }
    fn read_u16(&self, off: usize) -> u16 {
        let mut b = [0u8; 2];
        if let Some(s) = self.buf.get(off..off + 2) {
            b.copy_from_slice(s);
        }
        u16::from_le_bytes(b)
    }
    fn write_u16(&mut self, off: usize, v: u16) {
        if let Some(s) = self.buf.get_mut(off..off + 2) {
            s.copy_from_slice(&v.to_le_bytes());
        }
    }
