#!/usr/bin/env bash
# regen.sh — rebuild the LAYER2 pic-loader and regenerate pic-loader.bin.
#
# Pipeline (see Cargo.toml header for why each flag):
#   1. cargo +nightly build the standalone nyx-pic-loader cdylib for
#      x86_64-pc-windows-gnu with -Zbuild-std:
#        * --cfg poly1305_force_soft / chacha20_force_soft  → scalar crypto,
#          no cpufeatures writable flag, no AVX2
#        * -nostdlib → no mingw CRT (no IAT thunks; memcpy stays local)
#        * -Wl,-e,nyx_layer2_entry + --gc-sections → LTO dead-strips CRT
#        * -Zbuild-std-features=compiler-builtins-mem → local memcpy/memset
#   2. nyx-pic-dumper extracts the reachable closure from the export
#      `nyx_layer2_entry`, compacts it (entry prologue at offset 0), copies
#      the referenced .rdata constants, re-patches every displacement, and
#      refuses to emit for any image with relocations/imports/writable-data
#      references.
...
#
# Output: crates/nyx-loader/pic-loader/pic-loader.bin (committed; the emitter
# embeds it via include_bytes!).
#
...
RUSTFLAGS="-Cpanic=abort \
  ...
  -Clink-arg=-nostdlib \
  -Clink-arg=-Wl,-e,nyx_layer2_entry \
  -Clink-arg=-Wl,--gc-sections"

echo "==> building nyx-pic-loader (nightly, x86_64-pc-windows-gnu, -Zbuild-std)"
...
DLL="$TARGET/x86_64-pc-windows-gnu/release/nyx_pic_loader.dll"
...
echo "==> building nyx-pic-dumper"
cargo +nightly build --release --manifest-path "$MANIFEST" -p nyx-pic-dumper

DUMPER="$TARGET/release/nyx-pic-dumper"

echo "==> extracting reachable PIC closure -> pic-loader.bin"
"$DUMPER" "$DLL" "$OUT"
...
echo "==> done: $(wc -c < "$OUT" | tr -d ' ') byte pic-loader.bin"
