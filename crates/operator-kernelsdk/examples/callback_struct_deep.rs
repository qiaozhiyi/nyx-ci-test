...
fn looks_like_function_prologue(b: &[u8]) -> bool {
    ...
    if b[0] == 0x40 && b[1] >= 0x50 && b[1] <= 0x57 { return true; } // REX push
    // 4C 8B (REX.WR mov r...,...)
    if b[0] == 0x4C && b[1] == 0x8B { return true; }
    // 41 5x (push r8-r15)
...
fn main() {
    println!("[callback_struct_deep] READ-ONLY: find real routine offset in _PS_NOTIFY_ROUTINE_BLOCK");
    enable_privileges();
    ...
    for i in 0..notify_routines::ARRAY_LEN {
        ...
        for off in (0..0x40).step_by(8) {
            let v = match krw.kread_u64(ctx + off) { Ok(v) => v, Err(_) => continue };
            if !looks_like_kexec_ptr(v) { continue; }
            // 读 v 指向的前 16 字节
            let mut code = [0u8; 16];
            ...
            println!("  ctx+0x{:02x} = 0x{:016x}  -> [{} ] {}",
                     off, v, hex,
                     if is_func { "  <<< FUNCTION PROLOGUE (likely the real Routine)" } else { "" });
        }
        println!();
        if i >= 2 { break; } // 前3个 slot 够分析
    }
