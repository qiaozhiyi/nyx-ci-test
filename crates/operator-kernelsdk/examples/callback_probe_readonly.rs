...
fn main() {
    ...
    println!("ntoskrnl base=0x{:016x} PspCreateProcessNotifyRoutine=0x{:016x}", base, array_kva);
    ...
    for (i, packed, ctx) in &slots {
        ...
        for off in (0..0x40).step_by(8) {
            ...
            let flag = if looks_like_kexec_ptr(v) { " <-- looks like kexec ptr" } else { "" };
        ...
        println!("  >> telemetry.rs assumption (routine=*(ctx+0)=0x{:016x}) {}",
                 assumed_routine, if assumed_ok { "PLAUSIBLE" } else { "WRONG — not a kexec ptr" });
        // 如果错误，找出 ctx 内哪个偏移才像 routine。
        if !assumed_ok {
            for off in (0..0x40).step_by(8) {
                let v = krw.kread_u64(ctx + off).unwrap_or(0);
                if looks_like_kexec_ptr(v) {
                    println!("     * candidate routine at ctx+0x{:02x} = 0x{:016x}", off, v);
                }
            }
        }
        println!();
