...
| client-cli | `cargo test -p nyx-cli` 全过（含 116 集成测试）|
| implant capability | implant 交叉编译绿 + 真机 selftest exit code（见下）|
| kernel SDK | `cargo test -p nyx-operator-kernelsdk`（82 单测全过）|
...
[2] cargo test --workspace       : ✅326/0 / ❌(通过数，差额)
[3] clippy -p nyx-cli -D warnings: ✅零警告 / ❌(警告数)
